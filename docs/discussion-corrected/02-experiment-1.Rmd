...
takeOut <- c('500','600','700')
# experiments with these agentNumbers have much less simulations thereore not comparable with others;
agentNumbers <- setdiff(agentNumbers,takeOut)
i = 1
...
takeOut <- c('500','600','700')
# experiments with these agentNumbers have much less simulations thereore not comparable with others;
all.df <- all.df[which(!(all.df$agentNumber %in% takeOut)), ]

decentralized.df <- all.df[which(all.df$simulationType != "Centralized"), ]
centralized.df <- all.df[which(all.df$simulationType != "Decentralized"), ]
summ_dec.df <- ddply(decentralized.df,~agentNumber+similarityConnectThreshold,summarise,avg_vertices_total=mean(vertices_total),avg_sum_wallTime_min_total=mean(sum_wallTime_min_total))
summ_centr.df <- ddply(centralized.df,~agentNumber+similarityConnectThreshold,summarise,avg_vertices_total=mean(vertices_total),avg_sum_wallTime_min_total=mean(sum_wallTime_min_total))

dec <- ggplot(summ_dec.df, aes(x=avg_vertices_total, y=avg_sum_wallTime_min_total, color=similarityConnectThreshold)) + 
  geom_point( ) + 
  geom_smooth(method="loess", se=F) + 
  scale_x_continuous(labels=fancy_scientific) +
  ...
  #xlim(c(0, 0.1)) + 
  #ylim(c(0, 500000)) + 
  labs(title = 'Decentralized',
      y = 'simulationTime, minutes',
       x = 'number of vertices in the graph') +
  guides(size=FALSE)

centr <- ggplot(summ_centr.df, aes(x=avg_vertices_total, y=avg_sum_wallTime_min_total, color=similarityConnectThreshold)) + 
  geom_point( ) + 
  geom_smooth(method="loess", se=F) + 
  scale_x_continuous(labels=fancy_scientific) +
  #geom_text(aes(label=paste(agentNumber,'\nagents')), size=4) +
  #xlim(c(0, 0.1)) + 
  #ylim(c(0, 500000)) + 
  labs(title='Centralized',
    y = 'simulationTime, minutes',
       x = 'number of vertices in the graph') +
  guides(size=FALSE)

library(ggpubr)
figure <- ggarrange(centr,dec,
            common.legend=TRUE,
            legend='bottom')
figure
#annotate_figure(figure, bottom = text_grob("y-axis: simulationTime in minutes; x-axis: number of similarity edges in the graph;", size = 10))
summ.df <- ddply(all.df,~agentNumber+similarityConnectThreshold,summarise,avg_vertices_total=mean(vertices_total),avg_sum_wallTime_min_total=mean(sum_wallTime_min_total),avg_edges_total=mean(edges_total))
topology <- ggplot(summ.df, aes(x=avg_vertices_total, y=avg_edges_total, color=similarityConnectThreshold)) + 
  geom_point( ) + 
  geom_smooth(method="loess", se=F) + 
  scale_x_continuous(labels=fancy_scientific) +
  #geom_text(aes(label=paste(agentNumber,'\nagents')), size=4) +
